...
class PallasCallTCGen05Test(PallasTCGen05Test):
  ...
  def test_tmem(self, collective):
    transforms = self.default_transforms(dtype=jnp.float32)
    @self.kernel(
        out_type=jnp.zeros((128, 128), jnp.float32),
        scratch_types=[
  ...
  def test_tmem_allocation_estimation(self):
    ...
    @self.kernel(
        out_type=jnp.zeros((128, 256), jnp.bfloat16),
        scratch_types=[
...
class CoreMapTest(PallasTest, jtu.CudaArchSpecificTest):
  ...
  def test_multiple_wg(self):
    ...
    @self.kernel(
        out_type=jnp.zeros((2, 128), np.int32),
        num_threads=2,
        thread_name="wg",
    )
    def kernel(o_ref):
  ...
  def test_multiple_wg_with_grid(self):
    ...
    @self.kernel(
        out_type=jnp.zeros((4, 2, 128), np.int32),
        grid=(2, 2),
        grid_names=("x", "y"),
        num_threads=2,
        thread_name="wg",
  ...
  def test_cluster(self):
    ...
    @self.kernel(
        out_type=jnp.zeros(128, np.int32),
        grid=(2,),
        grid_names=("x",),
        cluster=(2,),
        cluster_names=("cluster",),
